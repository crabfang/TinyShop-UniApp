<template>
	<view class="rf-category">
		<uni-grid :column="4" :showBorder="false">
			<uni-grid-item v-for="fItem in categoryList">
			    <view class="category-list">
					<view class="category" @click="navToCategory(fItem.id)">
						<view class="img">
							<image :src="fItem.cover || errorImage" mode="aspectFill"></image>
						</view>
						<view class="text in1line">{{ fItem.title}}</view>
					</view>
				</view>
			</uni-grid-item>
		</uni-grid>
...
export default {
	...
	methods: {
		...
		// 跳转至分类模块
		navToCategory(id) {
			console.log("navToCategory: " + id)
			this.$mRouter.push({ route: `/pages/product/list?cate_id=${id}` });
		},
	}
};
</script>
